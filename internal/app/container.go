...
type Services struct {
	AuthLogin                    *authService.LoginService
	BookingCreateMy              *bookingService.CreateMyBookingService
	CustomerLineLogin            *customerService.LineLoginService
	CustomerLineRegister         *customerService.LineRegisterService
...
type Handlers struct {
	AuthLogin                    *authHandler.LoginHandler
	BookingCreateMy              *bookingHandler.CreateMyBookingHandler
	CustomerLineLogin            *customerHandler.LineLoginHandler
	CustomerLineRegister         *customerHandler.LineRegisterHandler
...
func NewContainer(cfg *config.Config, database *db.Database) *Container {
	...
	services := Services{
		AuthLogin:                    authService.NewLoginService(queries, cfg.JWT),
		BookingCreateMy:              bookingService.NewCreateMyBookingService(queries, database.PgxPool),
		CustomerLineLogin:            customerService.NewLineLoginService(queries, cfg.Line, cfg.JWT),
		CustomerLineRegister:         customerService.NewLineRegisterService(queries, database.PgxPool, cfg.Line, cfg.JWT),
	...
	handlers := Handlers{
		AuthLogin:                    authHandler.NewLoginHandler(services.AuthLogin),
		BookingCreateMy:              bookingHandler.NewCreateMyBookingHandler(services.BookingCreateMy),
		CustomerLineLogin:            customerHandler.NewLineLoginHandler(services.CustomerLineLogin),
		CustomerLineRegister:         customerHandler.NewLineRegisterHandler(services.CustomerLineRegister),
