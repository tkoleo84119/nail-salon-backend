...
type GetAllBrandsByFilterParams struct {
	Name     *string
	IsActive *bool
	Limit    *int
	Offset   *int
...
type GetAllBrandsByFilterItem struct {
	ID        int64              `db:"id"`
	Name      string             `db:"name"`
...
func (r *BrandRepository) GetAllBrandsByFilter(ctx context.Context, params GetAllBrandsByFilterParams) (int, []GetAllBrandsByFilterItem, error) {
	// where conditions
	...
	if total == 0 {
		return 0, []GetAllBrandsByFilterItem{}, nil
	}
	...
	var results []GetAllBrandsByFilterItem
