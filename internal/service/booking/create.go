...
func NewCreate(queries *dbgen.Queries, db *pgxpool.Pool, lineMessenger *utils.LineMessageClient, activityLog cache.ActivityLogCacheInterface) CreateInterface {
	return &Create{
		queries:       queries,
		db:            db,
...
func (s *Create) Create(ctx context.Context, req bookingModel.CreateParsedRequest, customerID int64) (*bookingModel.CreateResponse, error) {
	...
	go func() {
		...
		if err := s.activityLog.LogCustomerBooking(logCtx, customer.Name, utils.PgTextToString(customer.LineName), store.Name); err != nil {
